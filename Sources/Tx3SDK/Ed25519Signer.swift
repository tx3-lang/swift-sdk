...
///
/// The signer retains only the key object required for signing and never logs or includes key
/// material in errors. Mnemonic derivation intentionally belongs only to ``CardanoSigner``.
public struct Ed25519Signer: Signer {
    ...
    ///   contain exactly 32 bytes or CryptoKit rejects it.
    ...
    ///   odd-length, or non-32-byte hash input, or ``SigningFailure/cryptoFailure`` when CryptoKit
    ...
    public func sign(_ request: SignRequest) throws -> Witness {
        ...
        do {
            return Witness(
                publicKeyHex: signingKey.publicKey.rawRepresentation.lowercaseHex,
                signatureHex: try signingKey.signature(for: hash).lowercaseHex,
                type: .vkey
            )
        } catch {
